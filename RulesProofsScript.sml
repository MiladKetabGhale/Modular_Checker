...
val HWIN_thm = Q.store_thm("HWIN_thm",
  ...
  \\ PairCases_on`p` 
  \\ FULL_SIMP_TAC list_ss [HWIN_def,HWIN_dec_def,LENGTH_APPEND,NULL,NULL_EQ]     \\ rw[HWIN_def,HWIN_dec_def]
  \\ rw[HWIN_def,HWIN_dec_def]); 
   

val _ = mesonLib.max_depth := 20; 
	 ...
	 \\ fs[NULL,NULL_EQ,Functional_ElimAux_to_Logical,MEM,EQE_REMOVE_ONE_CAND,
	    ...
	    ORELSE 
	     (Cases_on`FLAT (get_cand_pile c p2)`
              >- (fs[NULL,NULL_EQ]
	       >> mesonLib.ASM_MESON_TAC [Functional_to_Logical_subpile2_backlog_trans2,MEM]) 
   ...
   >- (Cases_on `j2`      
    >- ((REPEAT STRIP_TAC >> PairCases_on `p` >> PairCases_on `p'` 
	  ...
	  >> metis_tac[NULL,NULL_EQ,Logical_TransferAux_to_Functional,
	       Functional_subpile1_IMP_TheLogical,
               Functional_subpile2_IMP_TheLogical,Functional_TransferAux_to_Logical,
	       TRANSFER_Auxiliary_dec_def,MEM,
	       subpile1_BlMem_trans2_IsCorrect,
...
val Logical_transfer_to_Functional_Transfer = Q.store_thm("Logical_transfer_to_Functional_Transfer",
 `! st qu l j1 j2. TRANSFER (qu,st,l) j1 j2 ==> TRANSFER_dec (qu,st,l) j1 j2`,
  ...
  REPEAT STRIP_TAC
   ...
   >> fs[Logical_TransferAux_to_Functional] 
   >>  ((`MEM c (MAP FST np)` by metis_tac[MEM_MAP,FST,TRANSFER_Auxiliary_def]
         ...
         >> mesonLib.ASM_MESON_TAC[Logical_subpile1_IMP_TheFunctional,Logical_subpile2_IMP_TheFunctional,
	    TRANSFER_Auxiliary_def,GET_CAND_PILE_MEM,Valid_PileTally_def])
	 ORELSE
	 (mesonLib.ASM_MESON_TAC[TRANSFER_Auxiliary_def,subpile1_BlMem_trans2_IsCorrect,MEM,
...
val Logical_TransferExcluded_to_Functional = Q.store_thm("Logical_TransferExcluded_to_Functional",
 `!qu st l j1 j2. TRANSFER_EXCLUDED (qu,st,l) j1 j2 ==> TRANSFER_EXCLUDED_dec (qu,st,l) j1 j2`,
   ...
   rw[] 
   >> fs[TRANSFER_EXCLUDED_def,TRANSFER_EXCLUDED_dec_def]     
     >> TRY (metis_tac[Logical_TransferExcludedAux_to_Functional]));
    ...
    rw[]
     >> TRY(Cases_on `j1`
	     ...
	     >- (PairCases_on`p` >> PairCases_on`p'` 
              >> fs[TRANSFER_EXCLUDED_def,TRANSFER_EXCLUDED_dec_def,NULL,NULL_EQ]       
              >> metis_tac[Functional_TransferExcluded_Aux_to_Logical])
	     >- fs[TRANSFER_EXCLUDED_dec_def])   
            >- fs[TRANSFER_EXCLUDED_dec_def]));
   ...
   `TRANSFER_EXCLUDED_dec = TRANSFER_EXCLUDED`,
    ...
    (simp[FUN_EQ_THM]
     ...
     >> Cases)
	  ...
	  >- (PairCases_on`p`
	    >> PairCases_on`p'`
	     >> mesonLib.ASM_MESON_TAC[TRANSFER_EXCLUDED_dec_def,TRANSFER_EXCLUDED_def,
	        Logical_TransferExcluded_to_Functional,Functional_TransferExcluded_to_Logical]) 
