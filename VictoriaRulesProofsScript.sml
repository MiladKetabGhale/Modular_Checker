...
val _ = new_theory "VictoriaRulesProofs";
...
val Functional_Elim_to_Logical_elim = Q.store_thm("Functional_Elim_to_Logical_elim",
 ...
 Cases_on `j1`
   >- (Cases_on `j2`    
	 ...
	 \\ fs[NULL,NULL_EQ,Functional_ElimAux_to_Logical,MEM,EQE_REMOVE_ONE_CAND,ALL_DISTINCT_APPEND]
	 \\ REPEAT CONJ_TAC)
 	   >- (Cases_on`FLAT (get_cand_pile c p2)`  
	      >- fs[]
	      >- fs[])   
	   >- (strip_tac
	    >> `?l0 ls. FLAT (get_cand_pile c p2) = l0 ::ls` by metis_tac [NON_Empty_list] 
	      >> fs[])
 ...
 `!qu st l j1 j2. TRANSFER_EXCLUDED (qu,st,l) j1 j2 ==> TRANSFER_EXCLUDED_dec (qu,st,l) j1 j2`,
  ...
  rw[] 
   >> fs[TRANSFER_EXCLUDED_def,TRANSFER_EXCLUDED_dec_def]     
     >> metis_tac[Logical_TransferExcludedAux_to_Functional]);
 
 
val Functional_TransferExcluded_to_Logical = Q.store_thm("Functional_TransferExcluded_to_Logical",
 `!qu st l j1 j2. TRANSFER_EXCLUDED_dec (qu,st,l) j1 j2 ==> TRANSFER_EXCLUDED (qu,st,l) j1 j2`,
...
val TRANSFER_EXCLUDED_thm = Q.store_thm("TRANSFER_EXCLUDED_thm",
   `TRANSFER_EXCLUDED_dec = TRANSFER_EXCLUDED`,
    simp[FUN_EQ_THM,Logical_TransferExcluded_to_Functional,Functional_TransferExcluded_to_Logical,FORALL_PROD,EQ_IMP_THM]);  
