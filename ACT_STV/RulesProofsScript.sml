 ...
 Cases_on `j1`   
   >- (Cases_on `j2`     
    >- ((REPEAT STRIP_TAC >> PairCases_on `p` >> PairCases_on `p'` 
	  ...
	  >> `!d. MEM d (h::t) ==> MEM d (MAP FST p2)` by metis_tac[TRANSFER_Auxiliary_dec_def,
           ...
           >> REPEAT CONJ_TAC)  
            >- metis_tac [Functional_TransferAux_to_Logical]
	    >- (`!d. MEM d (h::t) ==> MEM d (MAP FST p2)` by metis_tac[TRANSFER_Auxiliary_dec_def,
	       Logical_list_MEM_VICE_VERCA_TheFunctional,MEM,
               Valid_PileTally_def,PileTally_DEC1_to_PileTally,PileTally_DEC2_IMP_PileTally]
             >> metis_tac[Functional_AllNonEmpty_to_Logical,MEM])
	    >- metis_tac[NULL,NULL_EQ,Logical_TransferAux_to_Functional,Logical_list_MEM_VICE_VERCA_TheFunctional,
	              Functional_subpile1_IMP_TheLogical,MEM,
...
val Logical_transfer_to_Functional_Transfer = Q.store_thm("Logical_transfer_to_Functional_Transfer",
 `! st qu l j1 j2. TRANSFER (qu,st,l) j1 j2 ==> TRANSFER_dec (qu,st,l) j1 j2`,
   ...
   >>  `MEM c (MAP FST np)` by metis_tac[MEM_MAP,FST,TRANSFER_Auxiliary_def]
         ...
         >> (`!c. MEM c bl ==> MEM c (MAP FST p)` by metis_tac[TRANSFER_Auxiliary_def,Valid_Init_CandList_def,
                   ...
                   Logical_list_MEM_VICE_VERCA_TheFunctional,MEM])
          >>  REPEAT CONJ_TAC)
          >- metis_tac [TRANSFER_Auxiliary_def,
	    Logical_subpile1_IMP_TheFunctional,Logical_subpile2_IMP_TheFunctional,
            TRANSFER_Auxiliary_def,GET_CAND_PILE_MEM,Valid_PileTally_def,
	    Logical_list_MEM_VICE_VERCA_TheFunctional,PileTally_to_PileTally_DEC2,
	    ...
	    Logical_list_MEM_VICE_VERCA_TheFunctional,PileTally_to_PileTally_DEC2,
          ...
          >- (`MEM c (MAP FST np)` by metis_tac[MEM_MAP] 
		      ...
		      TRANSFER_Auxiliary_def])) 
   

(*
metis_tac [TRANSFER_Auxiliary_def,Logical_subpile2_IMP_TheFunctional,
            TRANSFER_Auxiliary_def,GET_CAND_PILE_MEM,Valid_PileTally_def,
	    Logical_list_MEM_VICE_VERCA_TheFunctional,PileTally_to_PileTally_DEC2,
                   Valid_PileTally_def,Valid_Init_CandList_def,MEM]
      ...
      >> fs[ELECT_dec_def,ELECT_def]    
       >> MAP_EVERY qexists_tac [`DROP (LENGTH p3) p'3`] 
        ...
        >> `!c. MEM c (DROP (LENGTH p3) p'3) ==> MEM c (MAP FST p'2)`
         ...
         >> metis_tac[Functional_ElectAux_to_Logical,ELECT_Auxiliary_dec_def,ALL_NON_EMPTY_def,
