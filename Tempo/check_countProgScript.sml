open preamble basis AuxEquivProofsTheory CheckerSpecTheory CheckerIMPTheory CheckerProofsTheory ParserProgTheory
...
val _ = overload_on("PARAMS_TYPE",
  ``(PAIR_TYPE RAT_TYPE
         (PAIR_TYPE NUM (LIST_TYPE AUXSPEC_CAND_TYPE)))``);

val loop_spec = Q.store_thm("loop_spec",
  `∀i iv j1 j1v j0 j0v fs.
   PARAMS_TYPE params pv ∧
   INT i iv ∧
   AUXSPEC_JUDGEMENT_TYPE j1 j1v ∧
   AUXSPEC_JUDGEMENT_TYPE j0 j0v
   ⇒
...
val check_count_spec = Q.store_thm("check_count_spec",
  ...
  \\ first_x_assum match_mp_tac
  \\ xfun`candidates_fun`
  \\ xapp_spec (Q.ISPECL[`LIST_TYPE AUXSPEC_CAND_TYPE`,`parse_candidates`]parse_line_spec)
  ...
  \\ first_x_assum match_mp_tac
  \\ xfun`j_fun`
  \\ xapp_spec (Q.ISPECL[`AUXSPEC_JUDGEMENT_TYPE`,`parse_judgement`]parse_line_spec)
