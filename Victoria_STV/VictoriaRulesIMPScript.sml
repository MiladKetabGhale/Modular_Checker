...
val _ = new_theory "VictoriaRulesBool";
...
val ELIM_CAND_dec_def = Define `
  (ELIM_CAND_dec c ((qu,st,l):params)
    ...
    /\ (NULL bl2) 
    /\ (case (FLAT (get_cand_pile c p)) of [] => bl2' = [] | _ => bl2' = [c])
    /\ (t = t') /\ (e = e')
    /\ (NULL bl) /\ (NULL bl') /\ (NULL ba)
...
val ELECT_dec = Define `
     (ELECT_dec ((qu,st,l): params)
           (NonFinal (ba, t, p, bl, bl2, e, h))
           (NonFinal (ba', t', p', bl', bl2', e',h')) <=>
		   (ELECT_Auxiliary_dec (qu,st,l) ba t p p' bl bl' e e' h h' (DROP (LENGTH bl) bl'))
                /\ (bl2 = bl2') /\ (bl2 = [])
                /\ (ba = []) /\ (ba' = [])
                /\ (t = t')
